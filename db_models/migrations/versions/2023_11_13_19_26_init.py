...
Revision ID: 23188c7911e6
Revises: 
Create Date: 2023-11-13 19:26:43.018189
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('updated_at', postgresql.TIMESTAMP(timezone=True), server_default=sa.text('now()'), nullable=False),
        sa.PrimaryKeyConstraint('id'),
        sa.UniqueConstraint('guard_name')
    )
    op.create_table(
        'shipping_classes',
        sa.Column('name', sa.String(), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column('type', sa.Enum('BILLING', 'SHIPPING', name='customeraddresstype'), nullable=True),
    ...
    op.create_table(
        'orders',
        sa.Column('tracking_number', sa.Integer(), server_default=sa.text("nextval('child_tracking_number_sequence')"),
                  nullable=False),
...
def downgrade() -> None:
    ...
    op.drop_table('customer_permission_association')
